    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //*******************针对Security的处理，start************************
                // 必须先有Controller的权限，然后才能有Handler方法的权限，所以有如下情况
                // 1 如果Controller和Handler都配置了@Security（取交集）
                // 2 如果仅仅Controller配置了@Security（以Controller为准）
                // 3 如果仅仅Handler方法配置了@Security（以Handler方法为准）
                if(aClass.isAnnotationPresent(Security.class) && method.isAnnotationPresent(Security.class)) {
                    ...
                    List<String> controllerUnameList = Arrays.asList(controllerUsernames);

                    Security handlerSecurity = method.getAnnotation(Security.class);
                    String[] handlerUsernames = handlerSecurity.value();
                    List<String> handlerUnameList = Arrays.asList(handlerUsernames);

                    // 求交集
                    controllerUnameList = new ArrayList<>(controllerUnameList);
                    handlerUnameList = new ArrayList<>(handlerUnameList);
                    controllerUnameList.retainAll(handlerUnameList);


                    securityMap.put( handler, controllerUnameList);
                    ...
                    String[] handlerUsernames = handlerSecurity.value();
                    List<String> handlerUnameList = Arrays.asList(handlerUsernames);
                    securityMap.put( handler, handlerUnameList);
                }
...
//        method.invoke() //
        ...
        if(usernameList != null && !usernameList.contains(req.getParameter("username"))) {
            resp.getWriter().write("sorry,the request has been forbidden---security");
            return;
        }
